 ...
 ***************************************************/
	(function () {
		...
		if (itemscroll.length) {
			...
			(function () {
				var gettheWidth = function gettheWidth() {
				...
				var initscrollitems = function initscrollitems(carousels) {
					...
					carousels.each(function () {
						...
						thisvar.jcarousel({
							scroll: howmany,
							itemVisibleInCallback: function itemVisibleInCallback() {
				...
				var adjustscrollitems = function adjustscrollitems() {
					...
					itemscroll.each(function () {
						...
						gettheWidth();
						if (thisvar.width() !== newWidth) {
							thisvar.css('width', newWidth).data('resize', 'true');initscrollitems(thisvar);thisvar.jcarousel('scroll', 1);
							var timer = window.setTimeout(function () {
								window.clearTimeout(timer);thisvar.data('resize', null);
							}, 600);
						}
					});
				};

				var resescrollitems = function resescrollitems(elem) {
					if (elem.data('resize')) {
						elem.css('left', '0');
					}
				};
				...
				jQuery(window).on('resize', function () {
					var timer = window.setTimeout(function () {
						window.clearTimeout(timer);adjustscrollitems();
					}, 30);
				});
			})();
		}
 ...
 ***************************************************/
	...
	(function () {
		...
		$(window).scroll(function () {
			...
			if (position > settings.min) $(settings.button).fadeIn(settings.fadeIn);else $(settings.button).fadeOut(settings.fadeOut);
		});
	})();
});
